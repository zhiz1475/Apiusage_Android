...
app.whenReady().then(async()=>{
 const root=process.cwd(); const win=new BrowserWindow({width:412,height:915,show:true,frame:false,webPreferences:{contextIsolation:true,nodeIntegration:false,sandbox:true,backgroundThrottling:false}});
 ...
 await sleep(1000);
 await win.webContents.executeJavaScript("(()=>{if(typeof setTheme==='function')setTheme('night',false);if(typeof setMode==='function')setMode('full');return true})()");
 await sleep(1000);
 ...
 const img=await win.webContents.capturePage(); fs.mkdirSync(path.join(root,'dist','mobile-preview'),{recursive:true}); fs.writeFileSync(path.join(root,'dist','mobile-preview','full-night-visible-412.png'),img.toPNG()); win.destroy(); app.quit();
});
